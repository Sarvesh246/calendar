...
- Phone chrome top cluster is always Assistant · Search · Filters · More. `MobileMoreMenu` is rooms only (Schedule, Settings). Focus is a named control on the Today header only (desktop + phone), not in More or on Calendar/Agenda. Hide completed lives in the Filter sheet only (not Settings). Schedule is reached via More (or the desktop rail), not as a peer tab.
- Settings cards: Everyday preferences (always open), Account & sync (with Install), Classes, Import, Weekly meetings (link to `/schedule`), Reminders, Look, Backup & export, Reset. Class times home is Schedule (`Class times` sheet); Settings only links there. Onboarding offers Import + Add class times as equal first steps.
...
- Settings and Schedule are phone "rooms" with a Done header (`MobileRoomHeader`) that returns to the remembered last tab (else `settings.landingView` / Today).
