...
- Phone chrome top cluster is always Assistant · Search · More (`MobileMoreMenu`): Filters, Full schedule, Settings, Focus. Filter is not a standalone top icon; hide completed toggles in the Filter sheet (+ Settings preference), not in More. Schedule is reached via More (or the desktop rail), not as a peer tab or Agenda button.
